...
| `@BindingMethods` / `@BindingConversion` | partial | Bucket confirmed during Discovery based on adapter chain result: if resolved adapter and conversion are both mechanical → mechanical; if conversion chain is ambiguous or type mismatch → escalate | adapter-resolution.md "Overload selection and @BindingConversion" |
