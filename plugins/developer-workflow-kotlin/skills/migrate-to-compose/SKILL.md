...
description: "Use for migrating Android View-based UI to Jetpack Compose — whether source is an Activity, Fragment, or standalone custom View/ViewGroup. Triggers: \"migrate to Compose\", \"convert to Compose\", \"rewrite in Compose\", \"move to Compose\", \"replace XML layout\", \"drop this fragment\", \"convert this Activity\", \"replace this custom view\", \"rewrite this ViewGroup\", or asks how to approach View→Compose migration for a specific screen. Covers: analyzing existing View-based implementation regardless of host (Activity/Fragment/custom View), discovering Compose patterns from other screens, identifying shared components, capturing visual baseline via screenshot, implementing the Compose equivalent, verifying visual fidelity and structural consistency. Do NOT use for: migrating entire apps at once (do screen-by-screen), KMP migrations (use kmp-migration), non-UI code rewrites."
---
