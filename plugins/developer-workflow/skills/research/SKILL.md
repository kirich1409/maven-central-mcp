...
4. One line: suggested next step in plain language (e.g. "ready to plan implementation" or "needs a written spec first").
...
|-----------|-----------------|
| Feature is large, multiple independent parts | Break it into smaller tasks before planning |
| Feature is clear, single task, ready to build | Drop into Plan Mode and start implementing |
| Complex approach, needs validation before coding | Plan Mode → `/multiexpert-review` |
| Research revealed a bug, not a feature need | Switch framing to a bug fix |
...
> **Next step:** feature splits into 3 independent parts → recommend planning each part separately
> before any coding.
