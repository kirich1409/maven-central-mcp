...
```mermaid
flowchart LR
    research["/research"] --> planReview["/multiexpert-review"]
...
- **gh CLI / glab CLI** — обязательно для `create-pr` и `drive-to-merge`. `drive-to-merge` не деградирует без CLI: без рабочей авторизации скилл не может автономно пушить фиксы, постить ответы и резолвить треды; при отсутствии CLI — попросит установить и авторизоваться, затем повторно запустить
