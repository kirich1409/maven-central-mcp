...
- **Testing** — framework (JUnit 4/5, Kotest), mocking (MockK / fakes), coroutine testing (`runTest`, Turbine), assertion lib, naming convention. Pick the framework using the canonical algorithm in the `developer-workflow:write-tests` skill, § Framework detection (build-file → existing tests → match module → platform default). Default for Android/Kotlin JVM when no signal exists: JUnit 5 + MockK. KMP default: `kotlin.test`. Never introduce a new framework without asking.
