...
Canonical values used throughout this skill.

- **`project_type`** — one of: `android`, `ios`, `web`, `desktop`, `backend-jvm`,
  `backend-node`, `cli`, `library`, `generic`. Detected from build files and source layout.
...
Detect `project_type`, `has_ui_surface`, and `ecosystem` from the build files
(`build.gradle*`, `package.json`, `Cargo.toml`, `pyproject.toml`, `go.mod`, `Package.swift`,
`*.xcodeproj`) and the source-tree layout. Default to `generic` when no signal matches
...
| Situation | Proposal |
|---|---|
| No spec, no test plan, code change ready | Run `/write-spec` (requirements doc) or `/generate-test-plan` (tests only), then re-run acceptance. |
...
| Bugfix path with no reproduction notes | Capture reproduction steps in plan mode (or `swarm-report/<slug>-debug.md`), then re-run acceptance. |
...
1. **Create the missing source** — produce the spec/test plan/reproduction notes (in plan
   mode or by invoking the proposed skill), then re-run acceptance.
...
The `code-reviewer` skipping here is decoupled from the Re-verification Loop's `diff_hash`
policy (see [`references/re-verification.md`](references/re-verification.md)) — the dedup
here is about "finalize already ran code-review on this diff", whereas `diff_hash`
idempotency is about "previous acceptance run covered this same diff".
...
On fix-loop re-entry (after `FAILED` → fix the code → re-run acceptance), compute
