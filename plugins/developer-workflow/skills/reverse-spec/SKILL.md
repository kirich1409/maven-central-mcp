...
the feature: [in scope]. Dependencies: [deps]. Out of scope: [out]. Confirm these
boundaries?"*. Adjust based on the answer before proceeding. A wrong boundary at this
...
If any of these trip, surface to the user: *"Scope covers [N flows / N features / N
screens] — that looks more like a set of features than one. Do you want one large
spec or per-flow specs? Recommendation: split — each separate spec is more useful
to a reimplementer than an aggregate."*. A large aggregate spec produces a Phase 1
state file and §10.1 Network operations table that are unusable in practice. Split
first, then spec each piece.
...
*"Found in-progress state for feature `<slug>` (Phase N, queue: K open questions).
Continue from there or start over?"*. Default is **continue**; restart wipes answers
already given and is rarely what the user wants.
...
and offer to override: *"Defaulting to <language> for the spec (matches the rest of
the project's docs). Different language?"* A single yes/no check is enough; do not
escalate if the user says nothing.
...
Override: *"skip project-overview"* skips this phase entirely.
...
the user can make an informed choice about pacing: *"Found ~N gaps after analysis.
I'll ask one question per round (each follow-up is informed by the previous answer).
Say 'batch' if you'd rather get them all at once."*. This prevents the common failure
mode where the user patiently answers 10 questions in a row, then at the end says
"could have done that in one message".
...
3. Ask it, with a **pre-filled default** derived from the code or from project
   ...
   requirement)"*. The user accepts, overrides, or says "don't know" (which itself
   is a valid answer and goes into Open Questions).
...
to batch ("ask them all at once"), switch to batched mode for the remainder. Always
...
- Delete the state file (`./swarm-report/reverse-spec-<slug>-state.md`) — operational,
  no longer needed.
- Offer to commit: *"Spec saved to `docs/spec/<slug>.md`. Commit it?"*. Do not commit
...
- **Hygiene artefact (optional).** If the analysis surfaced implementation-level
  ...
  findings to `docs/spec/<slug>-hygiene.md` (N items) — this is team backlog, not
  feature behavior."* If no hygiene findings exist, mention briefly (*"Hygiene
  artefact: not needed."*) and do not create the file.
- **Project overview update flag.** If Phase 0.6 surfaced discrepancies between the
  ...
  *"In `docs/project-overview.md`, field <X> may be stale — observed <Y>. I didn't
  update it — out of scope for the current feature."*. The skill never silently
  edits the project overview.
